...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    user: 'rudra@exoticaexperience.in',
    pass: 'Rudrasahasra@07'
  }
});
...
// Helper function to get Firebase db instance
function getFirestore() {
  return admin.firestore();
}

// Function to handle getting rejected faculty (for Vercel)
export async function getRejectedFaculty(req, res) {
  // Enable CORS
  res.setHeader('Access-Control-Allow-Origin', '*');
...
// Function to handle resending to rejected faculty (for Vercel)
export async function resendToRejected(req, res) {
  // Enable CORS
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type');
